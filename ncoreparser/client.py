...
class Client:
    ...
    def search(self, pattern, type=SearchParamType.ALL_OWN, where=SearchParamWhere.NAME,
               sort_by=ParamSort.UPLOAD, sort_order=ParamSeq.DECREASING, page_start=1, page_end=1) -> tuple[list[Torrent], int]:
        torrents = []
        ...
        max_pages = self._page_parser.get_max_pages(request.text)
        return torrents, max_pages
